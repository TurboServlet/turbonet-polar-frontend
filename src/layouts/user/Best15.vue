...
const best15 = toRef(props, 'best15');

const sortedBest15 = computed(() => {
  return best15.value.sort((a, b) => b.score - a.score);
});
...
<template>
  <div>
    <h2 class="text-2xl mt-3.5 mb-3.5"> Best 15</h2>
    <div v-if="best15.length > 0" class="rating-composition">
      <div v-for="music in sortedBest15" :key="music.musicId">
            ...
            <div class="scores-single">
              ...
              <div class="info">
                ...
                <div class="second-line"><span :class="rankColor(music.achievement.toFixed(0) )"><span
                    class="rank-text">{{ music.scoreRank }}</span> <span
                    class="rank-num">{{ music.achievement.toFixed(1) }}%</span></span> <span
                    class="dx-change">{{ music.score }}</span></div>
              </div>
            </div>
          </div>
        </div>
