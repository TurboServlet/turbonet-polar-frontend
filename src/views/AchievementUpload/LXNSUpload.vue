...
const showLxnsTokenStatus = async () => {
  await sendGetRequest('/web/showLxnsStatus', true).then((response) => {
...
const updateLxnsToken = async () => {
  isSetBtnLoading.value = true
  const payload = {
    "token": token.value,
  }
  await sendPostRequest('/web/updateLxnsToken', payload, true).then((response) => {
    if (response.statusCode === 200) {
...
const deleteLxnsToken = async () => {
  isDelBtnLoading.value = true
  const payload = {}
  await sendPostRequest('/web/deleteLxnsToken', payload, true).then((response) => {
    if (response.statusCode === 200) {
  ...
  <div v-else>
    <div class="breadcrumbs text-sm">
      <ul>
        ...
        <li>{{ $t('menu.upload.lxnsUpload') }}</li>
      </ul>
    </div>
    <div class="mt-2"/>
    <div v-if="responseData.isImportTokenExist === false" role="alert" class="alert shadow-lg alert-warning">
      ...
      <div>
        ...
        <div class="text-xs">{{ $t('achievementUpload.warning.lxnsContent') }}</div>
      </div>
    </div>
    <div class="mt-2"/>
    <div v-if="responseData.isImportTokenExist === false" class="hero hero-content text-center">
      <div>
        ...
        <div class="mt-10"></div>
        <div class="flex">
          <button type="submit" @click="updateLxnsToken" :disabled="isSetBtnLoading" class="btn max-sm:btn-sm btn-active btn-primary flex-1 mr-2">
        ...
        <div class="text-center text-xs opacity-60 w-5/6 mx-auto">
          ...
          <router-link class="text-primary" target="_blank" to="/tos">{{ $t('termsofservice.title') }}</router-link> {{ $t('achievementUpload.uploadNotice.2') }}
        </div>
      </div>
    </div>
    <div v-else class="border-2 border-base-200 rounded-box p-4 flex flex-col">
      <div class="flex">
        ...
        <div class="flex flex-wrap gap-2 justify-end">
          <button @click="deleteLxnsToken" :disabled="isDelBtnLoading" class="btn btn-error btn-sm"> {{ $t('achievementUpload.button.delete') }}
      ...
      <div v-else class="overflow-x-auto">
        <table class="table table-zebra">
          ...
          <tr v-for="history in responseData.history" >
            ...
            <td><AchievementSaverStatusBadge :status="history.status" /></td>
          </tr>
          </tbody>
        </table>
      </div>
